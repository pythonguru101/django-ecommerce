...
ORDER_PRODUCTS_CLASS = apps.get_model(settings.ORDER_PRODUCTS_MODEL)
...
class OrderCreateView(FormView):
    ...
    def get_admin_mail_title(self, order):
        return MESSAGE_NEW_ORDER_ADMIN

    def get_customer_mail_title(self, order):
        return MESSAGE_NEW_ORDER_USER
    ...
    def notify_customer(self, order):
        ...
        mail = EmailMessage(self.get_customer_mail_title(order), msg,
                            settings.SERVER_EMAIL, [order.user.email])
        mail.content_subtype = 'html'
        mail.send()

    def form_valid(self, form):
        from apps.shop.signals import order_create
        ...
        messages.info(self.request, MESSAGE_SUCCESS)
        cart.empty()
