...
class Cart(list):
    ...
    def _get_product(self, product):
        ...
        try:
            return list(filter(lambda x: x.product.pk == product.pk, self))[0]
        except IndexError:
            return None
    ...
    def remove(self, product, quantity=None, **kwargs):
        ...
        if not kwargs.get('stop_signal', None):
            cart_remove.send(sender=self.__class__, item=item, quantity=quantity)
...
def get_cart(request):
    return getattr(request, REQUEST_NAMESPACE, None)
